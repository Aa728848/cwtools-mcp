...
export function listResources() {
  return [
    { uri: 'cwtools://knowledge/game', name: 'CWTools game knowledge', description: 'Current-game semantic catalog summary and stable evidence-routing guidance.', mimeType: 'application/json' },
    { uri: 'cwtools://knowledge/diagnostic-routing', name: 'CWTools diagnostic routing', description: 'Structured diagnostic routing guidance and suggested tools.', mimeType: 'application/json' },
...
function parseJsonResource(content: string, sourcePath: string): unknown {
  try { return JSON.parse(content) as unknown; }
  catch (error) { return { status: 'invalid', sourcePath, error: error instanceof Error ? error.message : String(error) }; }
}

async function readResourceData(host: HostServices, uri: string): Promise<unknown> {
  switch (uri) {
    case 'cwtools://knowledge/game': {
      const catalog = await host.lsp.executeCommand('cwtools.ai.getSemanticCatalog', [[], []], { timeoutMs: 10_000 }).catch(() => undefined);
      return queryGameKnowledge('paradox', catalog);
    }
    case 'cwtools://knowledge/diagnostic-routing':
      return { status: 'ready', routes: [analyzeDiagnosticKnowledge({ message: 'syntax error' }), analyzeDiagnosticKnowledge({ message: 'missing localisation key' }), analyzeDiagnosticKnowledge({ message: 'scope mismatch' }), analyzeDiagnosticKnowledge({ message: 'unknown' })] };
    case 'cwtools://knowledge/workflow-hints': return queryWorkflowHints();
    case 'cwtools://project/profile': {
      ...
      return read.exists ? parseJsonResource(read.content, profilePath) : { status: 'missing', profilePath, _hint: 'Run /init in the VS Code extension or create .cwtools/project/profile.json.' };
    }
